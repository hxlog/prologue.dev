...
**Route transitions**: every page wraps its content in `<RouteTransition>` (`src/components/route-transition.js`), a `react` `<ViewTransition exit="page-exit" default="none">`. It animates **exit only** — `.page-enter` already fades each page in on first paint, and a first paint is not a Transition, so an `enter` would fire on navigation but not on load and would leave two competing fades. CSS lives in `globals.css` next to the theme block. Two details there are load-bearing: `::view-transition { pointer-events: none }` (the snapshot overlays the live page and would eat the reader's next click), and `html:not(.theme-vt)::view-transition-old(root) { animation: none }` — the UA cross-fades the root on *every* view transition, which double-animates a route change; the theme toggle has its own root rule and so is excluded rather than overridden.

**There is deliberately no `loading.js`.** Every route here is static and prefetched, so a navigation resolves in the same commit as the click and there is nothing to suspend on — the fallback could never legitimately show. Measured: adding one for `/blog` moved the real page into `<div hidden id="S:0">` behind JavaScript (+1.0 KB gz on `/blog`, +3.0 KB gz on a post page) with **no `<noscript>` fallback**, so a JS-less reader got skeletons instead of the article. `ViewTransition` gives the same perceived polish at zero payload cost. Do not re-add one without re-measuring.

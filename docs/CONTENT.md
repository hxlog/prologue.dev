...
Obsidian treats a leading `/` as a **vault-root** path, so that link resolves
against wherever the vault starts. Rooting the vault at `data/` therefore makes
`/static/images/foo.jpg` resolve to `data/static/images/foo.jpg` — the real
file, with no link or junction anywhere in the vault. Rooting it at the
repository root instead would send the same link to `<repo>/static/…`, which
does not exist.

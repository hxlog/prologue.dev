 ...
 * one form.
 *
 * ## Deletion is a hard stop while a tag is in use
 *
 * The server refuses, and the refusal carries the count. Rather than hiding the
 * button, the button reports the number: an author who wants to remove a tag
 * needs to know how many posts to go and edit, and a disabled control teaches
 * them nothing.
 */
                    ...
                    <span className="flex flex-wrap items-center gap-1">
                      ...
                      {tag.aliases.map((alias) => (
                        <button
                          ...
                          className="inline-flex items-center gap-0.5 rounded-full bg-surface-3 px-2 py-0.5 font-mono text-[10px] text-muted transition-colors hover:text-red-500 disabled:opacity-40"
                        >
                          {alias}
              ...
              <button
                ...
                onClick={() => {
                  if (tag.total_posts > 0) {
                    setError(
                      `「${tag.label}」还被 ${tag.total_posts} 篇文章使用，无法删除。` +
                        "先在这些文章里去掉这个标签，再回来删除。"
                    );
                    return;
                  }
                  ...
                  run(tag.id, () => deleteTagAction(tag.id), "标签已删除。");
                }}
